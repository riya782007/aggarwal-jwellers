-- 0076 — POS bill save: skip zero-qty stock ledger rows.
-- Backorder / out-of-stock lines deduct 0 units. Inserting delta=0 violated
-- stock_adjustments_delta_nonzero and aborted the whole sale ("invoice also not saving").
-- Apply this on the live database (schema.sql already contains the same body).
...
begin
  ...
  for it in select * from jsonb_array_elements(p_items) loop
    ...
    v_deducted := least(v_qty, greatest(0, v_avail));
