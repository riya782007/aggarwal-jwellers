...
import { getOrdersPage, getEmployees } from "@/lib/supabase/queries";
...
export default async function SalesRecords({ searchParams }: { searchParams: { page?: string; q?: string; channel?: string; from?: string; to?: string; sort?: string } }) {
  ...
  const employees = await getEmployees({ includeDeleted: true });
  ...
  // Grand total shown must MATCH the invoice. Cash memo = no tax. GST bill: `total` is pre-tax and 3%
  // is added ON TOP — UNLESS the bill is pinned GST-inclusive (`gst_mode==='inclusive'`), in which case
  // `total` already contains the GST and we must NOT add it again (that was double-counting the tax).
  const withTax = (r: any) => {
    const t = r.total ?? 0;
    if (r.bill_type === "cash" || r.gst_mode === "inclusive") return t;
  ...
  return (
    <main className="p-4 sm:p-6 bg-cream/40 min-h-screen">
      ...
      <form action="/admin/sales" className="flex flex-wrap gap-2 mb-4 items-center">
        <input name="q" defaultValue={q} placeholder="Search customer / phone / invoice…" className="rounded-xl border border-sand bg-white px-4 py-2 text-sm outline-none focus:border-emerald flex-1 min-w-[160px]" />
      ...
      <div className="overflow-x-auto rounded-2xl border border-sand bg-white shadow-card">
        <table className="w-full text-sm">
          <thead className="bg-cream text-muted text-left"><tr>
            ...
            <th className="p-3">Channel</th><th className="p-3">Bill</th><th className="p-3">Sold by</th><th className="p-3">Status</th>
          ...
          <tbody>
            ...
            {rows.map((r: any) => (
              <tr key={r.id} className="border-t border-sand/60 hover:bg-cream/40">
                <td className="p-3"><Link href={`/admin/invoice/${r.id}`} className="text-emerald nav-link font-medium">{r.invoice_no || String(r.id).slice(0, 8).toUpperCase()} <Icon g="↗" className="inline-block align-middle w-[1em] h-[1em]" /></Link>{r.source_tag === "estimate"
                ...
                <td className="p-3 text-muted whitespace-nowrap">{r.sales_employee_id ? (empById.get(r.sales_employee_id) ?? "—") : "—"}</td>
