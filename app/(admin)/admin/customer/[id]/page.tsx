...
export default async function CustomerDetail({ params }: { params: { id: string } }) {
  ...
  const fld = "rounded-xl border border-sand bg-white px-3 h-11 text-[15px] outline-none focus:border-emerald w-full";
  const fldArea = "rounded-xl border border-sand bg-white px-3 py-2.5 text-[15px] outline-none focus:border-emerald w-full";
  ...
  return (
    <main className="p-4 sm:p-6 bg-cream/40 min-h-screen max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-white rounded-2xl p-4 shadow-card"><p className={kpiLabel}>Orders</p><p className="text-xl font-semibold mt-1">{orderCount}</p></div>
        <div className="bg-white rounded-2xl p-4 shadow-card"><p className={kpiLabel}>Total spent</p><p className="text-xl font-semibold mt-1 text-emerald">{formatPaise(totalSpent)}</p></div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-card">
          <p className={kpiLabel}>Outstanding <span className="text-[11px] opacity-70">(from bills)</span></p>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-card"><p className={kpiLabel}>GSTIN</p><p className="text-[15px] font-medium mt-1 break-all">{c.gstin || "—"}</p></div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-card mb-4 border border-rose/20">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <form action={recordPartyPaymentAction} className="flex flex-wrap items-center gap-2">
              ...
              <input name="amount" type="number" min="1" step="1" placeholder={`₹ (baaki ${formatPaise(outstanding)})`} required className="rounded-xl border border-sand bg-white px-3 h-11 text-[15px] outline-none focus:border-emerald w-44" />
              <select name="mode" defaultValue="cash" className="rounded-xl border border-sand bg-white px-3 h-11 text-[15px] outline-none focus:border-emerald">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-card">
          ...
          {canManage ? (
            <form action={upsertCustomerAction} className="space-y-3">
              ...
              <textarea name="address" defaultValue={c.address ?? ""} className={fldArea} rows={2} placeholder="Address" />
              <textarea name="notes" defaultValue={c.notes ?? ""} className={fldArea} rows={2} placeholder="Notes" />
              <button className="btn-primary px-5 py-3 text-[15px] font-medium">Save changes</button>
            </form>
          ) : (
        ...
        <div className="bg-white rounded-2xl p-5 shadow-card">
          ...
          {orders.length === 0 ? <p className="text-sm text-muted">No orders yet for this customer.</p> : (
            <div className="overflow-x-auto"><table className="w-full text-[15px]">
              <thead className="text-muted text-left"><tr><th className="py-1.5">Order</th><th className="py-1.5">Date</th><th className="py-1.5">Type</th><th className="py-1.5 text-right">Amount</th></tr></thead>
              <tbody>
                {orders.map((o: any) => (
