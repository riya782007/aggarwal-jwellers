  ...
  return (
    <main className="p-4 sm:p-6 bg-cream/40 min-h-screen max-w-4xl">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link href={`/shop/${p.category?.slug}/${p.sku}`} target="_blank" className="px-4 py-2 rounded-full bg-ink/5 text-ink text-sm hover:bg-ink/10">View on store <Icon g="↗" className="inline-block align-middle w-[1em] h-[1em]" /></Link>
        {can(session, "catalog.view") && <Link href={`/admin/products/${(p as any).id}`} className="px-4 py-2 rounded-full bg-ink text-white text-sm hover:bg-ink/90"><Icon g="⚙" className="inline-block align-middle w-[1em] h-[1em]" />Manage product</Link>}
        {can(session, "catalog.edit") && <Link href={`/admin/catalogue/${p.sku}`} className="px-4 py-2 rounded-full bg-ink/5 text-ink text-sm hover:bg-ink/10 inline-flex items-center gap-1"><Icon g="✎" className="w-3.5 h-3.5" />Edit</Link>}
