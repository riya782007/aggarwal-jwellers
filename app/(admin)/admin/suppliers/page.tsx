          ...
          <tbody>
            ...
            {rows.map((s: any) => (
              <tr key={s.id} className="border-t border-sand/60 align-top">
                <td className="p-3 font-medium"><Link href={`/admin/supplier/${s.id}`} className="text-emerald nav-link">{s.name} <Icon g="↗" className="inline-block align-middle w-[1em] h-[1em]" /></Link>{s.notes && <span className="block text-xs text-muted font-normal">{s.notes}</span>}</td>
