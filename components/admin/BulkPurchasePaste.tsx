...
export function BulkPurchasePaste({ suppliers }: { suppliers: { id: string; name: string; city?: string | null }[] }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-card mb-6 border border-gold/30">
      <h2 className="font-medium text-ink mb-1"><Icon g="⚡" className="inline-block align-middle w-[1em] h-[1em]" />Paste the whole bill</h2>
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        {msg && <p className={`text-sm ${msg.ok ? "text-emerald-dark" : "text-rose"}`}>{msg.text} {msg.link && <a href={msg.link} className="text-emerald nav-link">Open bill <Icon g="→" className="inline-block align-middle w-[1em] h-[1em]" /></a>}</p>}
