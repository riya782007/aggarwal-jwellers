...
import { BulkAddInventory } from "@/components/admin/BulkAddInventory";
...
export function AddInventoryTabs(props: { categories: Cat[]; subcategories?: Sub[]; styles?: Sub[]; variantOptions: VariantOptions; colorCodes: ColorCodeMap; lang?: Lang }) {
  const [tab, setTab] = useState<"quick" | "new" | "bulkrows" | "bulk">("new");
  const lang = props.lang ?? "en";
  return (
    <div>
      <div className="inline-flex flex-wrap rounded-full bg-cream p-1 mb-5">
        {([["new", "Single product"], ["bulkrows", "Bulk add"], ["quick", t(lang, "qaTab")], ["bulk", "Sheet / AI import"]] as const).map(([k, label]) => (
      ...
      {tab === "quick" ? (
        ...
        <BulkAddInventory categories={props.categories} subcategories={props.subcategories ?? []} styles={props.styles ?? []} />
      ) : (
