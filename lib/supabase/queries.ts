...
export async function getBoxGroups(): Promise<{ id: string; code: string; label: string; packQty: number; sku: string; name: string; stock: number; price: number; wholesale: number }[]> {
  ...
  // hide column or an *_override hasn't been migrated, the WHOLE query errors and the barcodes
  // page would show zero box QRs. Fall back to the always-valid core columns in that case.
  ...
  if (!rich.error && rich.data) {
    ...
    const basic = await sb.from("inventory_groups").select(BASIC).eq("status", "active").order("created_at", { ascending: false });
    data = (basic.data as any[]) ?? [];
  }
  return (data ?? []).map((g) => {
