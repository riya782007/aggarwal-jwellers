...
export async function validateVoucher(
  ...
  const { data: v } = await sb
    ...
    .ilike("code", code)
    .maybeSingle();
  if (!v || !(v as any).active)
    return { ok: false, discountPaise: 0, message: "This code isn't valid." };
  const now = Date.now();
  if ((v as any).starts_at && new Date((v as any).starts_at).getTime() > now)
    return {
      ok: false,
      discountPaise: 0,
      message: "This code isn't live yet.",
    };
  if ((v as any).ends_at && new Date((v as any).ends_at).getTime() < now)
    return { ok: false, discountPaise: 0, message: "This code has expired." };
  if ((v as any).channel !== "all" && (v as any).channel !== channel)
    return {
      ok: false,
      discountPaise: 0,
      message: "This code doesn't apply here.",
    };
  if (
    (v as any).usage_limit != null &&
    (v as any).used_count >= (v as any).usage_limit
  )
    return {
      ok: false,
      discountPaise: 0,
  ...
  if (itemsPaise < ((v as any).min_order ?? 0))
    return {
      ok: false,
      discountPaise: 0,
      message: `Add ₹${Math.ceil((((v as any).min_order ?? 0) - itemsPaise) / 100)} more to use this code.`,
    };
  let disc =
    (v as any).kind === "flat"
      ? Math.min((v as any).value, itemsPaise)
      : Math.round(
          (itemsPaise * Math.min(90, Math.max(0, (v as any).value))) / 100,
        );
  if ((v as any).kind === "percent" && (v as any).cap != null)
    disc = Math.min(disc, (v as any).cap);
  ...
  if (disc <= 0)
    return {
      ok: false,
      discountPaise: 0,
      message: "This code gives no discount on this bag.",
    };
  return {
    ok: true,
    discountPaise: disc,
    code,
    message: `Code ${code} applied — you save ₹${Math.round(disc / 100)}.`,
  };
}
...
export async function applyVoucherToOrder(
  ...
  const { data: redeemed } = await sb.rpc("redeem_voucher", {
    p_code: check.code,
  });
  ...
  await sb
    ...
    .insert({
      kind: "sales",
      ref_id: orderId,
      debit: check.discountPaise,
      note: `Voucher ${check.code}`,
    });
