 ...
 */
import { computePrices, type PricingFormula } from "./pricing";
...
export function liveOffer(baseWholesalePaise: number, formula: PricingFormula): Offer {
  const { retailPrice, mrp } = computePrices(baseWholesalePaise, formula);
