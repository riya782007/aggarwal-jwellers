...
export const DIVA_TOOLS: DivaTool[] = [
  ...
  { name: "convert_invoice", kind: "mutate", permission: "billing.gst", confirm: true, desc: "Convert a final estimate (non-GST bill) into a GST invoice (or open billing if no invoice given).", params: [{ name: "invoice", type: "string", desc: "invoice number" }, { name: "to", type: "string", desc: "target bill type (gst)" }] },
