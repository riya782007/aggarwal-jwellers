...
function prompt(p: ProductLike) {
  ...
  return [
    `You are the senior product copywriter for "AggarwalDIVA", a premium artificial/imitation jewellery brand (Sadar Bazar, Rui Mandi, Delhi; retail + wholesale).`,
    ...
    ``,
    `TITLE — MUST follow AggarwalDIVA's exact house style:  «{First name} {material/style descriptors} {jewellery type} with {included pieces}»`,
    ...
    ``,
    `DESCRIPTION (ethnic/bridal voice) — match AggarwalDIVA's voice EXACTLY, 70-120 words, in this order:`,
    `  a) Open: "Add royal elegance to your festive look with {the exact title you wrote} by AggarwalDIVA."`,
    ...
    `seo.metaTitle <= 60 chars (title + " | AggarwalDIVA"); seo.metaDescription <= 155 chars, compelling; seo.keywords 8-12 long-tail phrases like "kundan necklace set for wedding", "artificial jewellery online India", "bridal jewellery Delhi".`,
    `Return ONLY the JSON object, minified, no markdown.`,
  ].filter(Boolean).join("\n");
}

export function buildGateway() {
  // OpenAI is the PRIMARY writer (the owner sets OPENAI_API_KEY for high-quality AggarwalDIVA titles);
  ...
  return new AiGateway({
    primary: {
      ...
      run: async (call: any) => JSON.parse(await openaiChat({
        system: "You are AggarwalDIVA's product copywriter. Return only valid minified JSON.",
    ...
    secondary: {
      ...
      name: "groq",
      run: async (call: any) => JSON.parse(await groqChat({ system: "You are AggarwalDIVA's product copywriter. Return only valid minified JSON.", user: call._prompt, json: true })),
    },
